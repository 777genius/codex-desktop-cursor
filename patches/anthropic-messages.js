...
import { logAgentError, logAccountAssigned, logAccountStats, logModelResolution, logTrafficRequest, logTrafficResponse, } from "../request-log.js";
...
import { prepareAgentInvocation, sessionIdFromStdout } from "../cursor-turn.js";
...
import { anthropicToolOutputs, parseAnthropicFunctionTools, resolveToolChoice, } from "../tool-types.js";
...
function anthropicToolUse(call) {
    let input = {};
    try {
...
function structuredAnthropicResponse(opts) {
    const content = [];
    if (opts.result.text)
        content.push({ type: "text", text: opts.result.text });
...
async function writeStructuredAnthropicTurn(opts) {
    if (!opts.stream) {
        ...
        json(opts.res, 200, structuredAnthropicResponse({
            body: opts.body,
            id: opts.id,
            model: opts.model,
            result,
...
export async function handleAnthropicMessages(req, res, ctx, rawBody, method, pathname, remoteAddress) {
    ...
    if (config.useAcp && submittedToolOutputs.length > 0) {
        ...
        if (Array.isArray(last?.content) &&
            ...
            json(res, 400, {
                error: {
                    type: "invalid_request_error",
                    message: "A tool_result resume message cannot contain additional content blocks",
                },
            });
            return;
        }
    ...
    const cleanSystemText = typeof cleanSystem === "string"
        ? cleanSystem
        : (cleanSystem ?? [])
    ...
    const systemWithTools = [cleanSystemText, toolsText, toolInstruction]
        .filter(Boolean)
        .join("\n\n");
    const prompt = buildPromptFromAnthropicMessages(cleanMessages, systemWithTools);
    ...
    if (cleanSystem) {
        const sys = typeof cleanSystem === "string"
            ? cleanSystem
            : cleanSystem
    ...
    if (config.useAcp && submittedToolOutputs.length > 0) {
        const record = ctx.toolSessions.findByCallIds("anthropic", ownerKey, submittedToolOutputs.map((output) => output.callId));
        if (!record) {
            json(res, 409, {
                error: {
                    type: "invalid_request_error",
                    message: "Tool session is missing or expired; restart the conversation",
                },
            });
            return;
        }
        ...
        try {
            const result = await writeStructuredAnthropicTurn({
                ...
                run: (listener) => ctx.toolSessions.resume(record, submittedToolOutputs, listener),
            });
