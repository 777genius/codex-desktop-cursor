...
import { chatToolOutputs, parseOpenAiFunctionTools, resolveToolChoice, } from "../tool-types.js";
...
function chatToolCallWire(calls) {
...
function chatToolResponse(opts) {
    ...
    const calls = opts.result.status === "tool_calls"
...
async function writeStructuredChatTurn(opts) {
    if (!opts.stream) {
        ...
        json(opts.res, 200, chatToolResponse({
            id: opts.id,
            created: opts.created,
            model: opts.model,
            result,
    ...
    const writeChunk = (delta, finishReason = null, usage) => {
        opts.res.write(`data: ${JSON.stringify({
            id: opts.id,
...
export async function handleChatCompletions(req, res, ctx, rawBody, method, pathname, remoteAddress) {
    ...
    try {
        ...
        const choice = resolveToolChoice(parsedTools, body.tool_choice ?? body.function_call, {
    ...
    if (config.useAcp && submittedToolOutputs.length > 0) {
        const record = ctx.toolSessions.findByCallIds("chat", ownerKey, submittedToolOutputs.map((output) => output.callId));
        if (!record) {
            json(res, 409, {
                error: {
